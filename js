...
let imageMaxWidthErrorMessage = "Too big. Decrease the size of the Image.";
let imageMinWidthErrorMessage = "Can't Visible. Increase the size of the Image.";
...
function onIncrement() {
    if (originalImageWidth >= imageMaxWidth) {
        warningMessageElement.textContent = imageMaxWidthErrorMessage;
    } else {
...
function onDecrement() {
    if (originalImageWidth <= imageMinWidth) {
        warningMessageElement.textContent = imageMinWidthErrorMessage;
    } else {
        originalImageWidth = originalImageWidth - 5;
        let updatedImageWidth = originalImageWidth + "px";
